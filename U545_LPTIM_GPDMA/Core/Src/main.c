...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    /* Start interrupt on compare match (for PA5 toggle) */


      /* Start PWM output on CH1 (PC1) */
//      if (HAL_LPTIM_PWM_Start(&hlptim1,LPTIM_CHANNEL_1) != HAL_OK)
//      {
//        Error_Handler();
//      }

      HAL_LPTIM_PWM_Start_DMA(&hlptim1, LPTIM_CHANNEL_1, DiscontinuousSineCCRValue_Buffer, CCRValue_BufferSize);
  ...
  while (1)
  {
    /* CPU can sleep; PA5 is toggled in DMA callback */
  }
    /* USER CODE END WHILE */
...
static void MX_LPTIM1_Init(void)
{
  ...
  sConfig1.Pulse =  DiscontinuousSineCCRValue_Buffer[0];
